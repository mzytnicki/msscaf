...
allObject <- addExp(allObject, hicData, "HiC")
allObject <- addExp(allObject, bamData, "10X")
#saveRDS(allObject, file = "/Scratch/mazytnicki/Mother_1/allObject0.rds")
#allObject <- readRDS(file = "/Scratch/mazytnicki/Mother_1/allObject0.rds")
allObject <- estimateDistributions(allObject)
allObject <- cleanData(allObject)
...
allObject <- findBreaks(allObject, 0.01)
allObject <- splitChromosomes(allObject)
#saveRDS(allObject, file = "/Scratch/mazytnicki/Mother_1/allObject1.rds")
#allObject <- readRDS(file = "/Scratch/mazytnicki/Mother_1/allObject1.rds")
allObject <- findJoins(allObject, 0.05)
#saveRDS(allObject, file = "/Scratch/mazytnicki/Mother_1/allObject2.rds")
#allObject <- readRDS(file = "/Scratch/mazytnicki/Mother_1/allObject2.rds")
allObject <- scaffold(allObject)
writeXStringSet(DNAStringSet(allObject@sequences), "sequences.fa")
allObject@breaks %>% select(ref, bin) %>% mutate(bin = bin * resolution) %>% write_tsv("breaks.tsv", col_names = FALSE)
...
../CheckScaffolding/checkJoins /Scratch/mazytnicki/Mother_1/mapping.paf outJoins.txt 1000 1000
...
sbatch inspector.sh
srun --mem=10G -c 8 inspector.py -c mother_raw.cgt.fa -r /work/project/seqoccin/data/reads/nanopore/bos_taurus/trio1.mother.run?.fastq.gz -d nanopore --ref GCF_002263795.1_ARS-UCD1.2_genomic.fna -o inspector_raw/ --skip_base_error --skip_structural_error -t 8
```
...
tmp <- sampleTriangles(allObject@data[[3]]@interactionMatrix, allObject@data[[3]]@outlierBins, allObject@sizes, allObject@data[[3]]@parameters@maxLinkRange, allObject@data[[3]]@parameters@metaSize, 1000) %>% as_tibble()
tmp %>% group_by(index) %>% group_split() %>% map_dfr(smoothenDistribution) %>% mutate(distance = factor(distance)) %>% ggplot(aes(count, color = distance)) + geom_freqpoly()           
tmp %>% group_by(index) %>% group_split() %>% map_dfr(smoothenDistribution) %>% mutate(distance = factor(as.integer(trunc(distance / 2)))) %>% ggplot(aes(count, color = distance)) + geom_freqpoly()           
tmp %>% group_by(index) %>% group_split() %>% map_dfr(smoothenDistribution) %>% filter(count > 0) %>% group_by(distance) %>% group_split() %>% map("count") %>% map(as.integer()) %>% map(fitdistr, "negative binomial")
tmp1 <- tmp %>% group_by(index) %>% group_split() %>% map_dfr(smoothenDistribution) %>% filter(count > 0) %>% mutate(count = as.integer(round(count))) %>% group_by(distance) %>% group_split() %>% map("count")
tmp2 <- tmp1 %>% map_dfr(computeCornerDifferenceOffsets, object@parameters@distanceCount, object@parameters@maxLinkRange, TRUE, pb)
tmp3 <- tmp2 %>% group_by(distance) %>% group_split()
