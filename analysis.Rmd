...
allObject <- findBreaks(allObject, 0.0001)
allObject <- splitChromosomes(allObject)
allObject <- findJoins(allObject, 0.00001)
allObject <- scaffold(allObject)
writeXStringSet(allObject@sequences, "sequences.fa")
...
library("Biostrings")
devtools::load_all(".")
object <- makeSimObject(100)
data1  <- makeSplitData(100, 20, 3, 0.2, 50)
data2  <- makeSplitData(100, 20, 3, 0.2, 50)
object <- addExp(object, data1, "test1")
object <- addExp(object, data2, "test2")
object <- estimateDistributions(object)
object@data[[1]]@parameters@maxLinkRange <- 6
object@data[[2]]@parameters@maxLinkRange <- 6
object <- cleanData(object)
object <- findBreaks(object, 0.1)
object@data[[1]]@breaks@mapPlots$dummyRef
object@data[[1]]@breaks@changePlots$dummyRef
```
...
gaussianNoise <-
splitRefs <- c(1, 2)
splitPositions <- c(50, 50)
joinRef1 <- c(3, 4)
joinRef2 <- c(4, 5)
joinDistances <- c(1, 10)
object <- makeSimObject(1, 1, 100, 20, 5, 0.2, c(1), c(50), c(), c(), c())
object <- makeSimObject(1, 2, 100, 20, 5, 0.2, c(), c(), c(1), c(2), c(1))
#object <- makeSimObject(2, 5, 100, 20, 5, 0.2, c(1, 2), c(50, 50), c(3, 4), c(4, 5), c(1, 10))
```
